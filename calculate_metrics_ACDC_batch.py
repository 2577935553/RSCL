...
# method_list=['BRPG','BCP','PCCS','UCMT']
# method_list=['BRPG','UNIMATCH_V2']
pct_list=[5,10,20]
for pct in pct_list:
    path_data='/public/home/zhsy/data/zhsy_data'
    pre_name=f'RSCL/ACDC_{pct}pct'
    pred_data_path=os.path.join(path_data, pre_name, 'predictions')
    num_classes = 4



    name_list = pd.read_csv(os.path.join(data_directory, 'data_dir/test_subj_ACDC.csv'))['label_filenames'].tolist()
    gt_label_list = ['testing/{}'.format(i,i) for i in name_list]
    ...
    for gt_label_filename, pred_filename in zip(gt_label_list, pred_list):
        ...
        if dice[1]>0.1:
            ...
            # 2. 将包含单行数据的字典添加到列表中，而不是直接附加到DataFrame
