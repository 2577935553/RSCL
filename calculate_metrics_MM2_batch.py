...
for pct in pct_list:
    ...
    pre_name=f'RSCL/LA_{pct}pct'
    pred_data_path=os.path.join(path_data, pre_name, 'predictions')
    num_classes = 4



    name_list = pd.read_csv(os.path.join(data_directory, 'data_dir/test_LA.csv'))['label_filenames'].tolist()
    gt_label_list = ['/public/home/zhsy/data/MnM-2/all/{}'.format(i,i) for i in name_list]
    ...
    for gt_label_filename, pred_filename in zip(gt_label_list, pred_list):
        try:
            ...
            label_nii = nib.load(os.path.join(gt_label_filename))
            pixel_spacing = label_nii.header['pixdim'][1:4]
            label = label_nii.get_fdata().astype(np.int8)

            pred_nii = nib.load(os.path.join(pred_data_path, pred_filename))
            pred = pred_nii.get_fdata().astype(np.int8)
